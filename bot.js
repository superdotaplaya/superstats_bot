...
bot.on('ready', () => {
    ...
    bot.generateInvite([
        ...
        console.log("I am currently working in " + bot.guilds.size + " servers!")
        });
    bot.user.setPresence({ game: { name: 'Stat Collector In ' + bot.guilds.size + ' Servers!' } })
    setInterval(function () {
        bot.user.setPresence({ game: { name: 'Stat Collector In ' + bot.guilds.size + ' Servers!' } });
        console.log("Updated Rich Prescence since it has been 5 minutes")
    }, 300 * 1000);
...
bot.on('message', message => {
    ...
    if (message.channel.type == "dm") {
        bot.channels.get('590311052365922309').send("User @<" + message.author.id + "> " + message.content)
        console.log("Direct message recieved" + message.content)
    }
    ...
    if (message.channel.id == "265156286406983680" || message.channel.id == "265156322012561408") {
        return;
    }
